    ...
    *,
    ...
    if resume and os.path.isfile(ckpt_path):
        ...
        loss_sum: Dict[str, float] = ckpt.get(
            "loss_sum", {"sft": 0.0, "pt": 0.0, "ifl": 0.0}
        )
        tok_sum: Dict[str, int] = ckpt.get("tok_sum", {"sft": 0, "pt": 0, "ifl": 0})
        print(f"[RESUME] Loaded checkpoint from step {start_step}")
    else:
        start_step = 0
        loss_sum = {"sft": 0.0, "pt": 0.0, "ifl": 0.0}
        tok_sum = {"sft": 0, "pt": 0, "ifl": 0}

    # ---------------- 2) build projectors & dirs -----------------------------
    number_of_params = get_number_of_params(model)
    projector_cls = get_trak_projector(device)

    projectors = {kind: [] for kind in ["sft", "pt", "ifl"]}
    for dim in proj_dim:
    ...
    # ---------------- 4) main loop -------------------------------------------
    full_grads = {k: [] for k in ["sft", "pt", "ifl"]}
    projected_gr = {k: {d: [] for d in proj_dim} for k in ["sft", "pt", "ifl"]}
    ...
    for batch in tqdm(dataloader_iter, total=remaining_total, initial=start_step):
        ...
        pt_batch = {
            ...
            "labels": batch["y_input_ids"],
        }
        prepare_batch(pt_batch)

        if gradient_type == "adam":
            g_pt, loss_pt = obtain_gradients_with_adam(model, pt_batch, m, v)
        elif gradient_type == "sign":
            g_pt, loss_pt = obtain_sign_gradients(model, pt_batch)
        else:
            g_pt, loss_pt = obtain_gradients(model, pt_batch)
        full_grads["pt"].append(g_pt)
        pt_tokens = (pt_batch["labels"] != -100).sum().item()
        loss_sum["pt"] += loss_pt * pt_tokens
        tok_sum["pt"] += pt_tokens
        model.zero_grad()

        # ---- (c) IFL = SFT – PT ---------------------------------------------
        g_ifl = g_sft - g_pt
        full_grads["ifl"].append(g_ifl)
        loss_ifl = loss_sft - loss_pt
        loss_sum["ifl"] += loss_ifl * sft_tokens
        tok_sum["ifl"] += sft_tokens
    ...
    print("Finished IFL gradient collection ✅")
