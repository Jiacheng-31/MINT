...
def get_training_dataset(
    train_files: List[str], tokenizer, max_seq_length, sample_percentage=1.0, seed=0
):
...
def encode_with_instruction_format(example, tokenizer, max_seq_length):
    ...
    # 1) Construct prompt using PROMPT_DICT
    instr = example["instruction"].strip()
    inp = example.get("input", "").strip()
    if inp:
        prompt = PROMPT_DICT["prompt_input"].format(instruction=instr, input=inp)
    else:
        prompt = PROMPT_DICT["prompt_no_input"].format(instruction=instr)

    # 2) Concatenate prompt + output + eos
    full_text = prompt + example["output"].lstrip() + tokenizer.eos_token

    # 3) Tokenize
    tokenized = tokenizer(
        full_text, return_tensors="pt", max_length=max_seq_length, truncation=True
    )
    input_ids = tokenized.input_ids
    labels = input_ids.clone()

    # 4) Mask prompt portion to avoid computing loss
